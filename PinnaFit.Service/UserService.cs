...
using PinnaFit.Core.Models.Interfaces;
using PinnaFit.DAL;
...
namespace PinnaFit.Service
{
    public class UserService : IUserService
    {
        ...
        {
            var user = _iDbContext.Set<UserDTO>().Include("Roles").Include("Warehouse").FirstOrDefault(u => u.UserName == displayName && u.Enabled == true);
            var role = _iDbContext.Set<UsersInRoles>().Include("User").Include("Role").Where(u => u.UserId == user.UserId).ToList();
            if (_disposeWhenDone)
                Dispose();
            return user;
        ...
        public IEnumerable<UserDTO> GetAll()
        {
            ...
            try
            {
                piList = _iDbContext.Set<UserDTO>().Include("Roles").Include("Warehouse")
